...
namespace Services
{
    public class AuthenticationManager : IAuthenticationService
    {
        ...
        public async Task<IdentityResult> CreateUser(UserDtoForInsertion userDtoForInsertion)
        {
            var user = _mapper.Map<User>(userDtoForInsertion);

            var result = await _userManager.CreateAsync(user, userDtoForInsertion.Password);

            if(result.Succeeded)
            await _userManager.AddToRolesAsync(user, userDtoForInsertion.Roles);
            return result;
        }
